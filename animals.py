...
def main_frames(name):
    ...
    if name == 'snake':
        ...
        while 1:
            ...
            # finding contours
            contour_red, _ = cv2.findContours(
            ...
            dist = int(math.sqrt(pow((last_point_x - point_x), 2) +
                       pow((last_point_y - point_y), 2)))
            ...
            for i, j in enumerate(points):
                if (i == 0):
                    continue
                cv2.line(
                    blank_img, (points[i-1][0], points[i-1][1]), (j[0], j[1]), (0, 0, 255), 5)
            cv2.circle(blank_img, (last_point_x, last_point_y),
                       5, (10, 200, 150), -1)
            ...
            cv2.putText(frame7, str("Score - "+str(score)),
                        (250, 450), font, 1, (0, 255, 0), 2, cv2.LINE_AA)
